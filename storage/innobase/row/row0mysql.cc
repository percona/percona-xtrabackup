				...
				to release and reacquire dict_operation_lock */
{
	...
	/* We first check whether SYS_TABLESPACES table exists, then we can
	know what version(5.5 or 5.6) the data is coming from, cause there
	are no SYS_TABLESPACES and SYS_DATAFILES internal InnoDB tables. If
	it's 5.5, we don't delete from these two tables, or crash happens. */

	if (dict_check_if_table_exists("SYS_TABLESPACES", TRUE)) {
		err = que_eval_sql(info,
