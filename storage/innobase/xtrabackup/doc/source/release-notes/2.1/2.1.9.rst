...
----------
 ...
 The xtrabackup binaries now recognize a new :file:`my.cnf` option, ``open_files_limit``. The effect is the same as for the server: it changes the maximum number of file descriptors available to the xtrabackup process. The actual limit depends on the platform and ``ulimit`` settings. Bug fixed :bug:`1183793`.
