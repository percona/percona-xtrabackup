...
.. sourcecode::
   ...
   ./bin/mysqld.my: { "components": "file://component_keyring_file" }
/lib/plugin/component_keyring_file.cnf:{ "path": "/var/lib/mysql-keyring/keyring_file", "read_only": false }

The keyring_file component queries ``performance_schema.keyring_component_status`` and returns the keyring information. The component has no special requirements for backing up a database that contains encrypted InnoDB tablespaces. 
...
.. note:: |xtrabackup| attempts to read ``xtrabackup_component_keyring_file.cnf``. You can assign another keyring file component by passing the ``--component-keyring-file-config`` option. 
...
- Backup
  ...
  .. code-block:: bash
   ...
   For |Percona Server| versions higher than 8.0.15-6,
