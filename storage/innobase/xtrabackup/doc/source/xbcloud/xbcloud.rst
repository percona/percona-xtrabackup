...
Preparing an incremental backups
--------------------------------------------------------------------------------

To prepare a backup, you download the full backup:
...
This command downloads just the ``ibdata1`` table and the ``sakila/payment.ibd`` table from a full backup.
...
.. option:: --swift-storage-url
   ...
   xbcloud attempts to get object-store URL for given region (if any specified)
