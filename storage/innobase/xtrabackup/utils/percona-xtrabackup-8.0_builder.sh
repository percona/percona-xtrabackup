...
install_deps() {
    ...
    then
        ...
        if [ "${OS_NAME}" == "noble" -o "${OS_NAME}" == "resolute" ]; then
...
build_deb(){
    ...
    echo "tfn=\$(/usr/bin/mktemp -p \$(/usr/bin/mktemp -d /tmp/XXXXXXXX) call-home.XXXXXX.sh)" >> percona-xtrabackup"${postfix}"-84.postinst
    echo "cat <<'CALLHOME' > \$tfn" >> percona-xtrabackup"${postfix}"-84.postinst
    cat call-home.sh >> percona-xtrabackup"${postfix}"-84.postinst
    echo "CALLHOME" >> percona-xtrabackup"${postfix}"-84.postinst
    echo "bash +x \$tfn -f \"PRODUCT_FAMILY_PXB\" -v \"${VERSION}-${DEB_RELEASE}\" -d \"PACKAGE\" &>/dev/null || :" >> percona-xtrabackup"${postfix}"-84.postinst
    echo "rm -rf \$tfn" >> percona-xtrabackup"${postfix}"-84.postinst
