	...
	lsn_t				range_end)	/*!<in: end LSN */
{
	...
	bool ret = os_file_scan_directory(srv_data_home,
		[=] (const char *path, const char *name) mutable -> void {
			...
			if (!log_online_is_bitmap_file(&statinfo,
						       name,
						       &file_seq_num,
						       &file_start_lsn)
	...
	ret = os_file_scan_directory(srv_data_home,
		[=] (const char *path, const char *name) mutable -> void {
			ulong	file_seq_num;
			lsn_t	file_start_lsn;
			size_t	array_pos;
			...
			if (file_seq_num >
				bitmap_files->files[array_pos].seq_num) {

				bitmap_files->files[array_pos].seq_num =
					file_seq_num;
				strncpy(bitmap_files->files[array_pos].name,
					name, FN_REFLEN);
				bitmap_files->files[array_pos]
					.name[FN_REFLEN - 1] = '\0';
