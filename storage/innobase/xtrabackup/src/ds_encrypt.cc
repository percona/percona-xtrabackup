...
static ds_ctxt_t *encrypt_init(const char *root) {
  ...
  ctxt = (ds_ctxt_t *)my_malloc(PSI_NOT_INSTRUMENTED,
                                sizeof(ds_ctxt_t) + sizeof(ds_encrypt_ctxt_t),
                                MYF(MY_FAE));
...
static ds_file_t *encrypt_open(ds_ctxt_t *ctxt, const char *path,
  ...
  file = (ds_file_t *)my_malloc(PSI_NOT_INSTRUMENTED,
                                sizeof(ds_file_t) + sizeof(ds_encrypt_file_t),
...
static int encrypt_write(ds_file_t *file, const void *buf, size_t len) {
  ...
  while (len > 0) {
    ...
    for (i = 0; i < nthreads; i++) {
      ...
      chunk_len = (len > XB_CRYPT_CHUNK_SIZE) ? XB_CRYPT_CHUNK_SIZE : len;
      thd->from = ptr;
...
static crypt_thread_ctxt_t *create_worker_threads(uint n) {
  ...
  threads = (crypt_thread_ctxt_t *)my_malloc(
      PSI_NOT_INSTRUMENTED, sizeof(crypt_thread_ctxt_t) * n, MYF(MY_FAE));
  ...
  for (i = 0; i < n; i++) {
    ...
    thd->to = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED,
                                 XB_CRYPT_CHUNK_SIZE + XB_CRYPT_HASH_LEN,
                                 MYF(MY_FAE));

    thd->iv =
        (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, encrypt_iv_len, MYF(MY_FAE));
    ...
    if (pthread_create(&thd->id, NULL, encrypt_worker_thread_func, thd)) {
...
static void *encrypt_worker_thread_func(void *arg) {
  ...
  while (1) {
    ...
    if (xb_crypt_encrypt(thd->cipher_handle, thd->from, thd->from_len, thd->to,
                         &thd->to_len, thd->iv)) {
      thd->to_len = 0;
      continue;
    }
