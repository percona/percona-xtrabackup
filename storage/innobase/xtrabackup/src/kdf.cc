...
gcry_error_t _gcry_kdf_pkdf2(const void *passphrase, size_t passphraselen,
  ...
  sbuf = (char *)(secmode ? gcry_malloc_secure(saltlen + 4 + hlen + hlen)
                          : gcry_malloc(saltlen + 4 + hlen + hlen));
  if (!sbuf) return gpg_err_code_from_syserror();
  tbuf = sbuf + saltlen + 4;
  ubuf = tbuf + hlen;

  ec = gcry_md_open(&md, hashalgo,
...
bool xb_derive_key(const char *passphrase, size_t passphraselen,
  ...
  gpg_error = _gcry_kdf_pkdf2(passphrase, passphraselen, GCRY_MD_SHA256, salt,
                              saltlen, XB_KDF_ITERATIONS, keysize, keybuffer);
...
bool xb_derive_key(const char *passphrase, size_t passphraselen,
  ...
  gpg_error = gcry_kdf_derive(passphrase, passphraselen, GCRY_KDF_PBKDF2,
                              GCRY_MD_SHA256, salt, saltlen, XB_KDF_ITERATIONS,
                              keysize, keybuffer);
