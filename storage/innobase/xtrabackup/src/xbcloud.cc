...
{
    slo_manifest *manifest = (slo_manifest *)(calloc(1, sizeof(slo_manifest)));
    if (manifest != NULL) {
        my_init_dynamic_array(&manifest->chunks, sizeof(slo_chunk), 50, 50);
    }
    return manifest;
}
...
int parse_args(int argc, char **argv)
{
    ...
    if (opt_storage == SWIFT) {
        ...
        if (opt_container == NULL) {
            fprintf(stderr, "container for Swift is not specified\n");
            return 1;
        }
        if (opt_url == NULL) {
           ...
           swift_auth_info *info)
{
    ...
    if (curl != NULL) {
        ...
        hdr_buf = (char *)(calloc(14 + max(strlen(username), strlen(key)), 1));
        ...
        if (revents & EV_READ) {
            ...
            if (nbytes > 0) {
                gcry_md_write(conn->md5,
                              conn->buffer + conn->filled_size, nbytes);
...
static int swift_upload_read_cb(void *ptr, size_t size, size_t nmemb, void *data)
{
    ...
    if (conn->filled_size == conn->upload_size &&
        ...
        if (nbytes > 0) {
            gcry_md_write(conn->md5, conn->buffer + conn->filled_size, nbytes);
            conn->filled_size += nbytes;
        } else {
            conn->global->eof = 1;
        }
    ...
    if (conn->upload_size == conn->buffer_size ||
        ...
        if (!conn->chunk_uploaded && realsize == 0) {
            ...
            hex_md5(gcry_md_read(conn->md5, GCRY_MD_MD5), conn->hash);
                                 ...
                                 global_io_info *global)
{
    ...
    if (conn != NULL) {
        ...
        conn->slist = curl_slist_append(conn->slist, "Content-Length: 0");
                   ...
                   const char *name)
{
    ...
    snprintf(url, array_elements(url), "%s/%s/%s", auth->url, container, name);
    snprintf(auth_token, array_elements(auth_token), "X-Auth-Token: %s",
             auth->token);
