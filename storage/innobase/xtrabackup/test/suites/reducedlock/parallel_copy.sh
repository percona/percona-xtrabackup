...
if ! egrep -q '[1-3][0-9] .*Copying file with space_id [0-9]+ .*test/tb_[0-9]*\.ibd to .*test/tb_[0-9]*\.ibd\.new' "$topdir/backup.log"; then
    die "xtrabackup did not copy tables in parallel"
fi
