							...
							checksum OK */
{
	...
	success = xb_os_file_read(bitmap_file->file, page, bitmap_file->offset,
							...
							file */
{
	...
	bitmap_file->file
		= xb_file_create_no_error_handling(bitmap_file->name,
						   OS_FILE_OPEN,
						   OS_FILE_READ_ONLY,
						   &success);
	if (UNIV_UNLIKELY(!success)) {
	...
	lsn_t				lsn)		/*!<in: LSN to find */
{
	...
	*page_end_lsn = 0;
	...
	while ((*page_end_lsn <= lsn)
		...
		*page_end_lsn = MACH_READ_64(page + MODIFIED_PAGE_END_LSN);
	}
...
/*=====================*/
{
	...
	while (last_page_ok
		...
		current_page_end_lsn
			= MACH_READ_64(page + MODIFIED_PAGE_END_LSN);
		last_page_in_run
			= mach_read_from_4(page + MODIFIED_PAGE_IS_LAST_BLOCK);
	}
