...
bool Vault_credentials_parser::is_valid_option(
    const Secure_string &option) const
{
    ...
    Vault_credentials::Map *vault_credentials)
{
  ...
  {
    err_ss << "Could not parse credential file. Cannot find equal sign (=) "
              "in line: ";
    err_ss << line_number << '.';
                                     ...
                                     Vault_credentials *vault_credentials)
{
  ...
  Secure_string        file_content(reinterpret_cast<char *>(buffer.get()),
                                    file_size);
  ...
  for (Vault_credentials::Map::iterator iter=
       ...
       iter != vault_credentials_in_progress.end(); ++iter)
  {
    ...
    if (value_options[iter->first]->process_value_options(
            iter->first, iter->second, logger))
      return true;
  }
