...
typedef std::map<std::string, keyring::Secure_string> option_value_container;
    ...
    Vault_credentials &vault_credentials) const
{
  ...
  if (!ifs.seekg(0, std::ios_base::beg))
  {
    logger_->log(MY_ERROR_LEVEL,
                 "Could not reset credentials file position.");
    return true;
  }
  ...
  while (!buffer.eof())
  {
    ...
    if (parse_line(logger_, line_number, line, option_values))
      return true;
    line_number++;
  }
  ...
  if (fnd != en)
  {
                 ...
                 secret_mount_point_version_raw, extracted_version))
    {
      ...
      {
        case 1: secret_mount_point_version= Vault_version_v1; break;
        case 2: secret_mount_point_version= Vault_version_v2; break;
        default:
        {
          err_ss << *option_name
  ...
  if (!vault_ca.empty() && vault_url_is_http)
  {
    err_ss << option_labels[option_vault_ca] << " is specified but "
           << option_labels[option_vault_url] << " is "
