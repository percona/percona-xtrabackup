    ...
    Vault_version_type &     vault_version)
{
  ...
  {
    // no "options" sections means we are using v1 server with only v1 MP available
  ...
  {
    // if "version" is a String, we try to convert it to an unsigned integer and check
    // the converted value to be 1 or 2
  ...
  // otherwise, when "version" is neither an Integer nor a String, report an error
  return true;
}
