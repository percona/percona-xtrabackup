...
/**
  ...
  Latest version of a key itself is stored somewhere else (presumably in keys_container).
...
public:
  ...
  /**
    ...
    For instance - when key's (argument key) id is system_key and latest version of system_key
...
private:
  static bool parse_system_key_id_with_version(std::string &key_id, std::string &system_key_id, uint &key_version);
  void update_system_key(IKey* key, const std::string &system_key_id, uint key_version);
