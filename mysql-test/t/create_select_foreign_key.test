...
--echo #
--echo # CASE 6 The behavior of CTAS, with sql_log_bin OFF and selecting
--echo # from a table containing data that fails a foreign key constraint
--echo #

SET sql_log_bin = OFF;
CREATE TABLE parent (f1 INT PRIMARY KEY);
CREATE TABLE child (f1 INT PRIMARY KEY, f2 INT,
                    FOREIGN KEY (f2) REFERENCES parent(f1));
...
--error ER_NO_REFERENCED_ROW_2
CREATE TABLE newchild (f1 INT PRIMARY KEY, f2 INT, 
                       FOREIGN KEY (f2) REFERENCES parent(f1))
                       AS SELECT f1, f2 FROM child;
