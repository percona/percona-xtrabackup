...
#
# 11. Usage with a procedure call which can return multiple result sets,
#     only the first result shall be considered.
...
--let $res=query_get_value(CALL p1(), a, 1)
--echo Fetched value from column 'a' of row 1 without specifying result set number is: $res
--let $res=query_get_value(CALL p1(), c, 2)
--echo Fetched value from column 'c' of row 2 without specifying result set number is: $res
--let $res=query_get_value(CALL p1(), a, 1, 1)
--echo Fetched value from column 'a' of row 1 of result set 1 is: $res
--let $res=query_get_value(CALL p1(), b, 2, 2)
--echo Fetched value from column 'b' of row 2 of result set 2 is: $res
--let $res=query_get_value(CALL p1(), c, 4, 3)
--echo Fetched value from column 'c' of row 4 of result set 3 is: $res

--error 1
--exec echo "let \$value= query_get_value(CALL p1(), c, 1, 6);" | $MYSQL_TEST 2>&1

SELECT 4 as regular_select;
DROP PROCEDURE p1;
DROP TABLE t1;
...
# declare all warnings
--let $warning_1_data_truncated = WARN_DATA_TRUNCATED
--let $warning_2_out_of_range = ER_WARN_DATA_OUT_OF_RANGE
--let $warning_3_division_by_zero = ER_DIVISION_BY_ZERO

# We have $ENABLED_WARNINGS_LIST - list of all enabled warnings 
# we have $DISABLED_WARNINGS_LIST - list of all disabled warnings

# $DISABLED_WARNINGS_LIST should be empty now
--echo "Disabled warnings: "
--echo $DISABLED_WARNINGS_LIST

# $ENABLED_WARNINGS_LIST
--echo "Enabled warnings: "
--echo $ENABLED_WARNINGS_LIST

--echo "Case-1 : No [enable/disable]_commands used"
INSERT INTO t1 VALUES (1, 'aaaaa', 32768, 1/0);
DELETE FROM t1;

--echo "Case-2 : disable all warnings"
# disable_warnings should disable all the warnings
--disable_warnings
INSERT INTO t1 VALUES (2, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-3 : enable all warnings"
# enable_warning should enable all the disable warnings
--enable_warnings
INSERT INTO t1 VALUES (3, 'aaaaa', 32768, 1/0);
DELETE FROM t1;
--echo "Enabled warnings (should be empty when all warnings are enabled): "
--echo $ENABLED_WARNINGS_LIST

--echo "Case-4 : specify a warning to disable"
# disable_warnings with warning list argument, should suppress only
# the specified warnings.
--disable_warnings $warning_1_data_truncated,$warning_2_out_of_range
--echo "Disabled warnings: "
--echo $DISABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (4, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-5 : Append the diabled warning list"
...
--disable_warnings $warning_3_division_by_zero
--echo "Disabled warnings: "
--echo $DISABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (5, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-6: specify a warning to enable"
--enable_warnings $warning_1_data_truncated
--echo "Enabled warnings: "
--echo $ENABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (6, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-7 : Append the enabled warning list"
--enable_warnings $warning_2_out_of_range
--echo "Enabled warnings: "
--echo $ENABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (7, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-8 : Enable all warnings"
--enable_warnings $warning_3_division_by_zero
--echo "Enabled warnings: "
--echo $ENABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (8, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-9 : Disable some warnings once"
--disable_warnings $warning_1_data_truncated,$warning_2_out_of_range ONCE
--echo "Disabled warnings: "
--echo $DISABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (9, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Disabled warnings: "
--echo $DISABLED_WARNINGS_LIST

# All warnings should appear 
INSERT INTO t1 VALUES (10, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--disable_warnings $warning_3_division_by_zero ONCE
--echo "Disabled warnings: "
--echo $DISABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (11, 'aaaaa',32768, 1/0);
DELETE FROM t1;
...
# All warnings should appear 
INSERT INTO t1 VALUES (12, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-10 : Enable some warnings once"
--disable_warnings
--enable_warnings $warning_1_data_truncated,$warning_2_out_of_range ONCE
--echo "Enabled warnings: "
--echo $ENABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (13, 'aaaaa',32768, 1/0);
DELETE FROM t1;
...
INSERT INTO t1 VALUES (14, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--enable_warnings $warning_3_division_by_zero ONCE
--echo "Enabled warnings: "
--echo $ENABLED_WARNINGS_LIST
INSERT INTO t1 VALUES (15, 'aaaaa',32768, 1/0);
DELETE FROM t1;
...
#no warnings should appear 
INSERT INTO t1 VALUES (16, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-11: Disabling some warning twice"
--enable_warnings
--disable_warnings $warning_1_data_truncated,$warning_2_out_of_range
INSERT INTO t1 VALUES (17, 'aaaaa',32768, 1/0);
DELETE FROM t1;

# below request would be ignored
--disable_warnings $warning_1_data_truncated
INSERT INTO t1 VALUES (18, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--echo "Case-12: Enabling some warning twice" 
--disable_warnings
--enable_warnings $warning_1_data_truncated,$warning_2_out_of_range
INSERT INTO t1 VALUES (19, 'aaaaa',32768, 1/0);
DELETE FROM t1;

--enable_warnings $warning_1_data_truncated
INSERT INTO t1 VALUES (20, 'aaaaa',32768, 1/0);
DELETE FROM t1;
...
--echo "Only $warning_2_out_of_range should be disabled once"
--disable_warnings $warning_1_data_truncated ONCE
--disable_warnings $warning_2_out_of_range ONCE
INSERT INTO t1 VALUES (1, 'aaaa', 32768, 1/0);
DELETE FROM t1;

--echo "All warnings shall be there"
INSERT INTO t1 VALUES (1, 'aaaa', 32768, 1/0);
DELETE FROM t1;

--echo "$warning_1_data_truncated, $warning_2_out_of_range should be disabled for the first INSERT statement." 
--disable_warnings $warning_1_data_truncated
--disable_warnings $warning_2_out_of_range ONCE
INSERT INTO t1 VALUES (1, 'aaaa', 32768, 1/0);
DELETE FROM t1;

--echo "Only $warning_1_data_truncated should be enabled."
INSERT INTO t1 VALUES (1, 'aaaa', 32768, 1/0);
DELETE FROM t1;
--enable_warnings

--echo "Only $warning_2_out_of_range should be enabled." 
--disable_warnings
--enable_warnings $warning_1_data_truncated ONCE
--enable_warnings $warning_2_out_of_range ONCE
INSERT INTO t1 VALUES (1, 'aaaa', 32768, 1/0);
DELETE FROM t1;
...
--echo "$warning_1_data_truncated, $warning_2_out_of_range should be enabled for the first INSERT statement"
--enable_warnings $warning_1_data_truncated
--enable_warnings $warning_2_out_of_range ONCE
INSERT INTO t1 VALUES (1, 'aaaa', 32768, 1/0);
DELETE FROM t1;
...
--echo #
--echo # Bug#29901754: ADD SUPORT FOR COLUMN MASKING TO DIFF_TABLES.INC
--echo #
...
--echo #
--echo #Bug#29769715 MTR: --REPLACE_* COMMANDS DON'T WORK WITH CAT_FILE COMMAND
--echo #

--write_file $MYSQLTEST_VARDIR/tmp/bug29769715.txt
abc 456.27
EOF
...
--echo #
--echo # Bug#33068833 MISSING END OF BLOCK ERROR WITH `--ASSERT` MYSQLTEST COMMAND
--echo #
...
--error 1
--exec echo "let \$false_var=0; assert(\$false_var);" | $MYSQL_TEST 2>&1

--write_file $MYSQLTEST_VARDIR/tmp/myt_assert1.test
--let $false_var=0
--assert(`SELECT $false_var`)
EOF
--error 1
--exec $MYSQL_TEST < $MYSQLTEST_VARDIR/tmp/myt_assert1.test 2>&1
--remove_file $MYSQLTEST_VARDIR/tmp/myt_assert1.test

--write_file $MYSQLTEST_VARDIR/tmp/myt_assert2.test
--let $myvar=1
if($myvar)
{
  --assert(`SELECT 0`)
...
EOF
--error 1
--exec $MYSQL_TEST < $MYSQLTEST_VARDIR/tmp/myt_assert2.test 2>&1
--remove_file $MYSQLTEST_VARDIR/tmp/myt_assert2.test

--write_file $MYSQLTEST_VARDIR/tmp/myt_assert3.test
--let $myvar=1
while($myvar)
{
  --assert($myvar)
...
EOF
--error 1
--exec $MYSQL_TEST < $MYSQLTEST_VARDIR/tmp/myt_assert3.test 2>&1
--remove_file $MYSQLTEST_VARDIR/tmp/myt_assert3.test

--write_file $MYSQLTEST_VARDIR/tmp/myt_assert4.test
--let $counter= 2
while ($counter)
{
  --assert($counter)
...
EOF
--error 1
--exec $MYSQL_TEST -x $MYSQLTEST_VARDIR/tmp/myt_assert4.test 2>&1
--remove_file $MYSQLTEST_VARDIR/tmp/myt_assert4.test
