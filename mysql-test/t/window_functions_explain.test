...
--source include/not_hypergraph.inc  # JSON format is different in hypergraph
...
# Display optimization and execution parts
# (preparation part exists only in non-ps mode).
# Note: JSON_PRETTY(JSON_EXTRACT()) is also a possibility but it shuffles keys
# which makes the trace have non-chronological order.
# Eliminate numbers which may vary:
--replace_regex /("peak_memory_used":) [0-9]+/\1 "NNN"/ /("key_size":) [0-9]+/\1 "XXX"/ /("row_size":) [0-9]+/\1 "XXX"/ /("max_rows_per_buffer":) [0-9]+/\1 "NNN"/
