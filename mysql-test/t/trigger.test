...
--error ER_STMT_NOT_ALLOWED_IN_SF_OR_TRG
create trigger t1_ai after insert on t1 for each row RESET REPLICA;
--error ER_STMT_NOT_ALLOWED_IN_SF_OR_TRG
...
CREATE TRIGGER t1_au AFTER UPDATE ON t1 FOR EACH ROW START REPLICA;

--error ER_COMMIT_NOT_ALLOWED_IN_SF_OR_TRG
CREATE TRIGGER t1_au AFTER UPDATE ON t1 FOR EACH ROW STOP REPLICA;
...
# Depends on the query plan. With "UPDATE t2 STRAIGHT_JOIN t1 ..."
# above, the old timestamp is seen with the old optimizer too.
...
--echo #
--echo # Testing bug#28492272
--echo #
--echo # Test case 1: SET sql_mode='time_truncate_fractional';
CREATE TABLE t1 (i INT, j VARCHAR(32));
SET sql_mode='time_truncate_fractional';

--echo # CREATE TRIGGER when sql_mode is set to time_truncate_fractional must
--echo # not assert
...
--echo # Test case 2 sql_mode=2147483648*2;
--echo # (This is actually equivalent to 'time_truncate_fractional', somehow)
CREATE TABLE t1 (i INT, j VARCHAR(32));
SET sql_mode=2147483648*2;

--echo # CREATE TRIGGER when sql_mode is set to 2147483648*2 must not trigger
--echo # assert
CREATE TRIGGER t1_before_insert BEFORE INSERT ON t1 FOR EACH ROW INSERT INTO
t3 VALUES (1, NEW.i, NULL, CONCAT("BI: ", NEW.j));
...
--echo #                Even trigger names are case and accent insensitive. Test case to
--echo #                verify the same.
--echo #
SET NAMES utf8mb3;
...
--echo #
--echo # Test case to verify triggers with case and accent insensitive names.
--echo #
CREATE TRIGGER cafe BEFORE INSERT ON t1 FOR EACH ROW SET @sum= @sum + NEW.f1;
--echo # Following statement errors out as trigger with name 'cafe' already
--echo # exists on the table.
--error ER_TRG_ALREADY_EXISTS
...
--echo # Trigger with NAME_LEN size name.
