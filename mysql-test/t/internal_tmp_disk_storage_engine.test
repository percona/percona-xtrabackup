...
# union distinct with global order by
  # filesort
--replace_regex $elide_costs
...
#system table
# Run analyze on the dictinary table, character_sets, to stabilize test
SET DEBUG='+d,skip_dd_table_access_check';
ANALYZE TABLE mysql.character_sets;
--disable_warnings
--replace_regex $elide_costs
...
#from heap to ondisk //ref wl6711_heap_to_disk.test

###################################################
#BUG: Table->read_set is not set
...
#BUG: row_id is too long to use unique_constraint for semi-join
...
analyze table t1,t3;
#Due to cost changes, materialzed scan will be preferred 
#for this query instead of dups_weedout.  
#So changing optimizer_switch.
set @old_optimizer_switch=@@optimizer_switch;
set optimizer_switch='materialization=off';
--replace_regex $elide_costs
...
# update t1, t3 set t3.b=10, t1.a=20;
...
SET sql_mode = 'NO_ENGINE_SUBSTITUTION';
--echo #
--echo # BUG#18542045: WL6737/WL6711:SELECT DISTINCT @VARIABLE
--echo #               WITH ORDER BY GIVING INCORRECT RESULTS          
--echo #
...
--echo #
--echo # BUG#18540717: WL6737/WL6711:DISTINCT AND 
--echo #               UNION DISTINCT QUERIES SHOWING RESULT MISMATCH   
--echo #
...
--echo #
--echo # BUG#18921514: WL6711: CRASH IN THD_TO_INNODB_SESSION
--echo #               AT HANDLER/HA_INNODB.CC 
--echo #
--disable_warnings
CREATE TABLE C (
                pk INTEGER AUTO_INCREMENT,
...
--echo #
--echo # BUG#18956748: WL6711: CRASH AT MY_RAW_FREE IN MYSYS/MY_MALLOC.C
--echo #
CREATE TABLE C (
                pk int NOT NULL AUTO_INCREMENT,
...
--echo #
--echo # BUG#18999867: WL6711::RESULT MISMATCH WHEN USING
--echo #                INNODB AS TMP ENGINE FOR SUBQUERY
--echo #
CREATE TABLE C (
                col_int_key int DEFAULT NULL,
                pk int NOT NULL AUTO_INCREMENT,
...
--echo #
--echo # BUG#19019765: WL6711:RESULT DIFF WITH INNODB AS
--echo #    TMP STORAGE FOR SUB-QUERIES WITH DERIVED TABLE
--echo #
CREATE TABLE BB (
                 pk int NOT NULL AUTO_INCREMENT,
                 col_int_nokey int NOT NULL,
...
DROP TABLE CC,C,BB;
--echo #
--echo # BUG#19067369: CRASH AT
--echo # ROW_SEL_CONVERT_MYSQL_KEY_TO_INNOBASE FOR SUBQUERY
--echo #
CREATE TABLE C (
                pk INTEGER AUTO_INCREMENT,
                col_int_nokey INTEGER NOT NULL,
...
--echo #
--echo # BUG#19274972: RESULT DIFF SEEN FOR QUERIES WITH
--echo #               DISTINCT AND ORDER BY
--echo #
CREATE TABLE t1 (
  col_varchar_10_latin1_key varchar(10),
  pk int NOT NULL,
...
--echo #
--echo # BUG#19191377: WL#6737: NOT NULL COLUMN IS BEING PROJECTED
--echo #              AS NULLABLE IN TEMPORARY TABLES.
--echo #
...
--echo #
--echo # BUG#20059644: WL#6737: CRASH AT OPERATOR<< IN DICT/DICT0MEM.CC 
--echo #
CREATE TABLE C (
  pk int NOT NULL AUTO_INCREMENT,
  col_varchar_key varchar(1) NOT NULL,
...
DROP TABLE C,CC;
--echo #
--echo # BUG#19627741: SELECT DISTINCT SUM() RETURNS DUPLICATE ROWS
--echo #
CREATE TABLE t1 (
a int DEFAULT NULL,
b int DEFAULT NULL,
...
--echo #
--echo # Bug#20454833 PREFIX INDEX FIELD, ON AN INAPPROPRIATE DATA TYPE
--echo #
...
--echo #
--echo # Bug#20506527 FAILING ASSERTION: !MBMAXLEN || !(PREFIX_LEN % MBMAXLEN)
--echo #
...
--echo #
--echo # Bug#20752543 INNODB:FAILING ASSERTION:FLAGS & BUF_PAGE_PRINT_NO_CRASH
--echo #
SET @old_big_tables=@@big_tables;
SET big_tables=on;
...
--echo #
--echo # Bug#20454979 CRASH IN ACTUAL_KEY_PARTS 
--echo #
...
--echo #
SHOW GLOBAL VARIABLES WHERE Variable_name LIKE '%\_storage\_engine'
AND Variable_name NOT LIKE 'external_table%';
#============================end && cleanup======================= 
set big_tables= @old_big_tables;
set optimizer_switch= @old_optimizer_switch;
...
--echo #
--echo # Bug #22343910: SELECT DISTINCT NOT RETURNING DISTINCT ROWS IN 5.7
--echo #
