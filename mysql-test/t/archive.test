...
--echo #
--echo # Bug#28923281: UPDATE ON JSON COLUMN IN ARCHIVE ENGINE HITS AN ASSERTION
--echo #
...
--echo #
--echo # Verify that all UPDATE and DELETE operations on archive
--echo # tables are rejected.
--echo #
...
--echo # Delete on empty tables should be rejected.
--error ER_ILLEGAL_HA
...
--echo # Update of empty tables should be rejected.
--error ER_ILLEGAL_HA
...
--echo # No-op updates should be rejected.
--error ER_ILLEGAL_HA
...
--echo # Actual updates should be rejected.
--error ER_ILLEGAL_HA
...
--echo # Allow multi-table updates if the archive table is only read from.
UPDATE archive_table AS t1, innodb_table AS t2
       SET t2.x = t2.x + 1 WHERE t1.id = t2.id;
SELECT * FROM archive_table ORDER BY id;
SELECT * FROM innodb_table ORDER BY id;

--echo # Delete should be rejected.
--error ER_ILLEGAL_HA
DELETE FROM archive_table;
--error ER_ILLEGAL_HA
DELETE FROM v;
...
--echo # Delete is allowed if the archive table is only read from.
