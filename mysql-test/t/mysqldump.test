...
--echo # Check that we don't dump in utf8mb3 in compatible mode by default,
--echo # but use the default compiled values, or the values given in
--echo # --default-character-set=xxx. However, we should dump in utf8mb3
...
--exec $MYSQL_DUMP --character-sets-dir=$MYSQL_SHAREDIR/charsets --default-character-set=utf8mb3 --tab=$MYSQLTEST_VARDIR/tmp/ test t1
--echo ##################################################
--cat_file $file
--echo ##################################################
TRUNCATE t2;
...
--echo #
--echo # Bug#35209008: mysqldump --column-statistics has invalid default value
--echo #   by default the option must be disabled
--echo #

--exec $MYSQL_DUMP column_statistics_dump t1 > $assert_file
...
--echo #
--echo # WL10905 - Support for INVISIBLE columns.
--echo #           Test case to verify Logical Dump using mysqldump tool
--echo #
...
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--exec $MYSQL_DUMP --databases test > "$MYSQLTEST_VARDIR/tmp/test1.sql" 2>&1
--exec $MYSQL_DUMP test t2 --tab "$MYSQLTEST_VARDIR/tmp" 2>"$MYSQLTEST_VARDIR/tmp/test_t2.log"
DROP TABLE t1, t2;

--exec $MYSQL --database test < $MYSQLTEST_VARDIR/tmp/t2.sql
SHOW CREATE TABLE t2;
DROP TABLE t2;

--exec $MYSQL < $MYSQLTEST_VARDIR/tmp/test1.sql
SHOW CREATE TABLE t1;
...
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD XML INFILE "$MYSQLTEST_VARDIR/tmp/tmp1.xml" INTO TABLE t2 (f1, f2);
SHOW CREATE TABLE t2;
SELECT f1, f2 FROM t2 ORDER BY f2;
...
--echo #
--echo # WL13784 - Create auto_increment PK on table creation when no PK is specified
--echo #
--echo # Test case to verify Logical Dump using mysqldump tool
--echo #
...
--echo # Test case to check table dump with generated primary key.
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--echo # Dump complete test database with generated invisible primary key
--echo # information using mysqldump tool.
--exec $MYSQL_DUMP --databases test > "$MYSQLTEST_VARDIR/tmp/test1.sql" 2>&1
--echo # Dump test.t3 table with generated invisible primary key information
--echo # using mysqldump tool.
--exec $MYSQL_DUMP test t3 --tab "$MYSQLTEST_VARDIR/tmp" 2>"$MYSQLTEST_VARDIR/tmp/test_t3.log"
DROP TABLE t1, t2, t3;

--echo # Load from the test.t3 dump created using mysqldump tool."
--exec $MYSQL --database test < $MYSQLTEST_VARDIR/tmp/t3.sql
--echo # Table t3 should list generated primary key information.
SHOW CREATE TABLE t3;
DROP TABLE t3;

--echo # Load from the complete test database dump created using mysqldump tool."
--exec $MYSQL < $MYSQLTEST_VARDIR/tmp/test1.sql
--echo # Table t1 should list generated primary key information.
SHOW CREATE TABLE t1;
SELECT * FROM t1;
SELECT my_row_id, f FROM t1;

--echo # Table t2 should list generated primary key information.
SHOW CREATE TABLE t2;
SELECT * FROM t2;
...
--echo # Test case to check table dump *without* generated primary key.
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--echo # Dump complete "test" database *without* generated invisible primary
--echo # key information using mysqldump tool.
--exec $MYSQL_DUMP --skip-generated-invisible-primary-key --databases test > "$MYSQLTEST_VARDIR/tmp/test2.sql" 2>&1
--echo # Dump test.t3 table *without* generated invisible primary key
--echo # information using mysqldump tool.
--exec $MYSQL_DUMP --skip-generated-invisible-primary-key test t3 --tab "$MYSQLTEST_VARDIR/tmp" 2>"$MYSQLTEST_VARDIR/tmp/test_t3.log"
DROP TABLE t1, t2, t3;

--echo # Load from the test.t3 dump created using mysqldump tool."
--exec $MYSQL --database test < $MYSQLTEST_VARDIR/tmp/t3.sql
--echo # Table t3 should *not* list generated primary key information.
SHOW CREATE TABLE t3;
SELECT * FROM t3;
DROP TABLE t3;
...
--echo # Table t2 should *not* list generated primary key information.
SHOW CREATE TABLE t2;
SELECT * FROM t2;
...
--echo # Test case to verify Logical Dump in XML format using mysqldump tool
--echo # Load from the complete test database dump created using mysqldump tool."
--exec $MYSQL < $MYSQLTEST_VARDIR/tmp/test1.sql
--echo # Dump test.t1 table in XML format using mysqldump tool.
--exec $MYSQL_DUMP --xml test t1 > "$MYSQLTEST_VARDIR/tmp/tmp1.xml" 2>&1
--echo # Dump test.t2 table in XML format using mysqldump tool.
--exec $MYSQL_DUMP --xml test t2 > "$MYSQLTEST_VARDIR/tmp/tmp2.xml" 2>&1
--echo # Dump test.t3 table in XML format using mysqldump tool.
--exec $MYSQL_DUMP --xml test t3 > "$MYSQLTEST_VARDIR/tmp/tmp3.xml" 2>&1
DELETE FROM t1;
DELETE FROM t2;
DELETE FROM t3;
--echo # For columns t1.my_row_id and t1.f, values should be loaded from the dump.
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD XML INFILE "$MYSQLTEST_VARDIR/tmp/tmp1.xml" INTO TABLE t1 (my_row_id, f);
SHOW CREATE TABLE t1;
SELECT * FROM t1;
SELECT my_row_id, f FROM t1;

--echo # For columns t2.my_row_id, t1.f1 and t1.f2, values should be loaded from the dump.
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD XML INFILE "$MYSQLTEST_VARDIR/tmp/tmp2.xml" INTO TABLE t2 (my_row_id, f1, f2);
SHOW CREATE TABLE t2;
SELECT * FROM t2;
SELECT my_row_id, f1 FROM t2;

--echo # For columns t3.my_row_id and t1.f1, values should be loaded from the dump.
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD XML INFILE "$MYSQLTEST_VARDIR/tmp/tmp3.xml" INTO TABLE t3 (my_row_id, f1);
SHOW CREATE TABLE t3;
SELECT * FROM t3;
SELECT my_row_id, f1 FROM t3;

--echo # Test case to check table dump *without* generated primary key in XML format.
--echo # Dump test.t1 table *without* generated invisible primary key information in XML format
--echo # using mysqldump tool.
--exec $MYSQL_DUMP --xml --skip-generated-invisible-primary-key test t1 > "$MYSQLTEST_VARDIR/tmp/tmp1.xml" 2>&1
--echo # Dump test.t2 table *without* generated invisible primary key information in XML format
--echo # using mysqldump tool.
--exec $MYSQL_DUMP --xml --skip-generated-invisible-primary-key test t2 > "$MYSQLTEST_VARDIR/tmp/tmp2.xml" 2>&1
--echo # Dump test.t3 table *without* generated invisible primary key information in XML format
--echo # using mysqldump tool.
--exec $MYSQL_DUMP --xml --skip-generated-invisible-primary-key test t3 > "$MYSQLTEST_VARDIR/tmp/tmp3.xml" 2>&1
DROP TABLE t1, t2, t3;

--echo # Dump file tmp1.xml contains data for only one column "f". For my_row_id,
--echo # default value is inserted.
CREATE TABLE t1 (f INT NOT NULL PRIMARY KEY, my_row_id INT DEFAULT 580030);
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD XML INFILE "$MYSQLTEST_VARDIR/tmp/tmp1.xml" INTO TABLE t1(f);
SELECT * FROM t1;

--echo # Dump file tmp2.xml contains data for column "f1" and "f2". For my_row_id,
--echo # default value is inserted.
CREATE TABLE t2 (f1 INT NOT NULL PRIMARY KEY, f2 INT, my_row_id INT DEFAULT 580030);
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD XML INFILE "$MYSQLTEST_VARDIR/tmp/tmp2.xml" INTO TABLE t2(f1, f2);
SHOW CREATE TABLE t2;
SELECT * FROM t2;

--echo # Dump file tmp3.xml contains data for only one column "f1". For my_row_id,
--echo # default value is inserted.
CREATE TABLE t3 (f1 INT NOT NULL PRIMARY KEY, my_row_id INT DEFAULT 580030);
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD XML INFILE "$MYSQLTEST_VARDIR/tmp/tmp3.xml" INTO TABLE t3(f1);
SHOW CREATE TABLE t3;
SELECT * FROM t3;

--echo # Test case to verify primary key generation while loading table dump
--echo # obtained *without* generated primary key on a MySQL server with
--echo # @@global.sql_generate_invisible_primary_key = ON.
SET GLOBAL sql_generate_invisible_primary_key = ON;

--echo # Load from the dump created without generated primary key information.
--exec $MYSQL < $MYSQLTEST_VARDIR/tmp/test2.sql
--echo # Primary key is generated for table t1 while loading.
--echo # Table t1 should list generated primary key information.
SHOW CREATE TABLE t1;
SELECT my_row_id, f FROM t1;

--echo # Primary key is generated for table t2 while loading.
--echo # Table t2 should list generated primary key information.
SHOW CREATE TABLE t2;
SELECT my_row_id, f1, f2 FROM t2;

--echo # Primary key is generated for table t3 while loading.
--echo # Table t3 should list generated primary key information.
SHOW CREATE TABLE t3;
SELECT my_row_id, f1 FROM t3;
...
remove_file $MYSQLTEST_VARDIR/tmp/t3.sql;
remove_file $MYSQLTEST_VARDIR/tmp/test_t3.log;
remove_file $MYSQLTEST_VARDIR/tmp/tmp1.xml;
remove_file $MYSQLTEST_VARDIR/tmp/tmp2.xml;
remove_file $MYSQLTEST_VARDIR/tmp/tmp3.xml;
DROP TABLE t1, t2, t3;
...
--echo #
--echo # WL#15662: Add options to mysqldump to skip views and utilize read ahead
--echo #
...
--exec $MYSQL_DUMP --no-create-info --compact --init-command="INSERT INTO init_command_db.t VALUES (1)" init_command_db
DELETE FROM init_command_db.t;

--echo "With --init-command and --init-command-add ==========================="
--exec $MYSQL_DUMP --no-create-info --compact --init-command="INSERT INTO init_command_db.t VALUES (1)" --init-command-add="INSERT INTO init_command_db.t VALUES (2)" --init-command-add="INSERT INTO init_command_db.t VALUES (3)" init_command_db
DELETE FROM init_command_db.t;

--echo "Only with --init-command-add ========================================="
--exec $MYSQL_DUMP --no-create-info --compact --init-command-add="INSERT INTO init_command_db.t VALUES (1)" --init-command-add="INSERT INTO init_command_db.t VALUES (2)" --init-command-add="INSERT INTO init_command_db.t VALUES (3)" init_command_db
DELETE FROM init_command_db.t;

--echo "With --init-command-add ovewritten by --init-command ================="
--exec $MYSQL_DUMP --no-create-info --compact --init-command-add="INSERT INTO init_command_db.t VALUES (1)" --init-command="INSERT INTO init_command_db.t VALUES (2)" --init-command-add="INSERT INTO init_command_db.t VALUES (3)" init_command_db
DELETE FROM init_command_db.t;

--echo "Test if the commands are executed in order ==========================="
--exec $MYSQL_DUMP --no-create-info --compact --init-command="SET @init_var_1=0"  --init-command-add="SET @init_var_1=99" --init-command-add="INSERT INTO init_command_db.t VALUES (@init_var_1)" init_command_db
...
--echo "Without --ignore-views ==============================================="
--exec $MYSQL_DUMP --compact --databases skip_views_db > $MYSQLTEST_VARDIR/tmp/mysqldump.sql
# restore and verify the view not exists
DROP DATABASE skip_views_db;
--exec $MYSQL test < $MYSQLTEST_VARDIR/tmp/mysqldump.sql
SHOW FULL TABLES IN skip_views_db WHERE TABLE_TYPE LIKE 'VIEW';

--echo "With --ignore-views =================================================="
--exec $MYSQL_DUMP --compact --databases --ignore-views skip_views_db > $MYSQLTEST_VARDIR/tmp/mysqldump.sql
# restore and verify the view not exists
DROP DATABASE skip_views_db;
--exec $MYSQL test < $MYSQLTEST_VARDIR/tmp/mysqldump.sql
SHOW FULL TABLES IN skip_views_db WHERE TABLE_TYPE LIKE 'VIEW';

--echo "Verify skip dumping views by setting option --ignore-views for mysqldump where DB1 has view and another DB2 don't have view."
--exec $MYSQL_DUMP --compact --ignore-views --databases skip_views_db skip_views_db2
...
create procedure mysqldump_test_db.sp1() select 'hello';
--disable_warnings
create library mysqldump_test_db.sp1 LANGUAGE JAVASCRIPT COMMENT 'Library Comment' AS " export function f(n) {  return '$$' } ";
create library mysqldump_test_db.sp2 LANGUAGE JAVASCRIPT COMMENT 'Library Comment' AS " export function f(n) {  return '$$' + '$JAVASCRIPT$' } ";
...
drop library mysqldump_test_db.sp2;

--echo # Import back the exported routines that were just deleted.
--exec $MYSQL test < $MYSQLTEST_VARDIR/tmp/mysqldump.sql
show create procedure mysqldump_test_db.sp1;
show create function mysqldump_test_db.sp1;
show create library mysqldump_test_db.sp1;
show create library mysqldump_test_db.sp2;
--remove_file $MYSQLTEST_VARDIR/tmp/mysqldump.sql
drop database mysqldump_test_db;

--echo #
--echo # WL#16358 Support for Libraries: test orphaned stored programs
--echo #

create database mysqldump_test_db;
--disable_warnings
create library mysqldump_test_db.lib1 LANGUAGE JAVASCRIPT AS " export function f1() {  return 1 } ";
create library mysqldump_test_db.lib2 LANGUAGE JAVASCRIPT AS " export function f2() {  return 2 } ";
create library mysqldump_test_db.lib3 LANGUAGE JAVASCRIPT AS " export function f3() {  return 3 } ";
create function mysqldump_test_db.sp1() RETURNS INTEGER LANGUAGE JAVASCRIPT
USING (mysqldump_test_db.lib1, mysqldump_test_db.lib2, mysqldump_test_db.lib3)
as " return lib1.f1() + lib2.f2() +  lib3.f3()";
--enable_warnings

drop library mysqldump_test_db.lib2;

show create function mysqldump_test_db.sp1;

--echo # Because of dropped lib2, sp1 cannot be recreated anymore
--error 2
--exec $MYSQL_DUMP --skip-comments --routines --databases mysqldump_test_db > $MYSQLTEST_VARDIR/tmp/mysqldump.sql
--echo # Console dump.
--error 2
...
--disable_warnings
create library mysqldump_test_db.lib2 LANGUAGE JAVASCRIPT AS " export function f2() {  return 22 } ";
--enable_warnings
--echo # Now that lib2 is there again, dump should work
--exec $MYSQL_DUMP --skip-comments --routines --databases mysqldump_test_db > $MYSQLTEST_VARDIR/tmp/mysqldump.sql

drop function mysqldump_test_db.sp1;
drop library mysqldump_test_db.lib1;
drop library mysqldump_test_db.lib2;
drop library mysqldump_test_db.lib3;

--echo # Import back the exported routines that were just deleted.
--exec $MYSQL test < $MYSQLTEST_VARDIR/tmp/mysqldump.sql

show create function mysqldump_test_db.sp1;
show create library mysqldump_test_db.lib1;
show create library mysqldump_test_db.lib2;
show create library mysqldump_test_db.lib3;
