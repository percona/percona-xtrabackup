...
# results was valid with 1.77 but invalid with 1.84
...
--echo #
--echo # Bug #22819614 OUT OF RANGE SRIDS ARE SILENTLY CAST TO UINT32
--echo #
...
--echo #
--echo # Bug #22883056 INCORRECT FUNCTION REFERENCED IN SPATIAL ERROR MESSAGE
--echo # ERROR 3037 (22023)
--echo #
...
--echo # WKB functions used on geometry values (MySQL extension)
...
--echo #
--echo # Bug #23573720 ASSERTION FAILED: GEOMETRY::IS_VALID_GEOTYPE(GT)
--echo #
...
--echo #
--echo # Bug #23280574 ASSERTION FAILED: !NO_DATA(NBYTES) IN
--echo # GEOMETRY::WKB_PARSER::SKIP_UNSAFE()
--echo #
...
--echo #
--echo # Bug #25582178 ASSERTION `LENGTH > 0' FAILED. IN SQL/FIELD.CC:8732
--echo #
...
--echo # Bug #23023817 ST_SIMPLIFY: INVALID READS IN
--echo # PROTOCOL_CLASSIC::NET_STORE_DATA
--echo #
...
--echo #
--echo # Bug #26615824 WL10827: SELECT QUERY WITH LIMIT 1 RETURNS ZERO
--echo # NUMBER OF ROWS
--echo #
...
--echo #
--echo # Bug #26412713 ASSERTION `THD->IS_ERROR()' FAILED IN SQL/SQL_INSERT.CC
--echo #

--echo # Inserting a NULL value in a GEOMETRY NOT NULL column should raise an
--echo # error, even if using INSERT IGNORE.
CREATE TABLE t1(g GEOMETRY NOT NULL);
--error ER_BAD_NULL_ERROR_NOT_IGNORED
...
--echo #
--echo # WL#2388 GeomCollection synonym for GeometryCollection
--echo #

--echo # Both GEOMCOLLECTION and GEOMETRYCOLLECTION are allowed as column type.
...
--echo # Only GEOMETRYCOLLECTION is allowed in WKT.

--error ER_GIS_INVALID_DATA
DO ST_GEOMFROMTEXT('GEOMCOLLECTION(POINT(0 0))');
DO ST_GEOMFROMTEXT('GEOMETRYCOLLECTION(POINT(0 0))');

--echo # The column type is GEOMCOLLECTION.

SELECT ST_GEOMETRYTYPE(GEOMETRYCOLLECTION(POINT(0, 0)));

--echo # The default function name is GEOMCOLLECTION.

EXPLAIN SELECT GEOMCOLLECTION();
EXPLAIN SELECT GEOMETRYCOLLECTION();

--echo #
--echo # Bug #26174808 BG::UNION_(MPY, PY) RETURNS EMPTY RESULT
--echo #

--error ER_POLYGON_TOO_LARGE
SELECT ST_Contains(
...
--echo #
--echo # Bug #27252609 UNABLE TO CREATE INDEX WHEN SPATIAL DATATYPES ARE USED
--echo # WITH REQUIRED OPTION
--echo #
...
--echo #
--echo # Bug#27756083 "INSERT INTO SELECT" QUERY INSERTS GEOMETRY OF DIFFERENT
--echo #              TYPE
--echo #
...
--echo # POINT should only be allowed in POINT and GEOMETRY column
...
--echo # MULTIPOINT should only be allowed in MULTIPOINT, GEOMETRY and
--echo # GEOMETRYCOLLECTION column
--error ER_CANT_CREATE_GEOMETRY_OBJECT
...
--echo # LINESTRING should only be allowed in LINESTRING and GEOMETRY column
--error ER_CANT_CREATE_GEOMETRY_OBJECT
...
--echo # MULTILINESTRING should only be allowed in MULTILINESTRING, GEOMETRY and
--echo # GEOMETRYCOLLECTION column
--error ER_CANT_CREATE_GEOMETRY_OBJECT
...
--echo # POLYGON should only be allowed in POLYGON and GEOMETRY column
--error ER_CANT_CREATE_GEOMETRY_OBJECT
...
--echo # MULTIPOLYGON should only be allowed in MULTIPOLYGON, GEOMETRY and
--echo # GEOMETRYCOLLECTION column
--error ER_CANT_CREATE_GEOMETRY_OBJECT
...
--echo # GEOMETRYCOLLECTION should only be allowed in GEOMETRYCOLLECTION and
--echo # GEOMETRY column
--error ER_CANT_CREATE_GEOMETRY_OBJECT
...
--echo #
--echo # Bug #27808412 UBSAN: ITEM_FUNC_AS_WKB::VAL_STR, NULL POINTER PASSED AS
--echo # ARGUMENT
--echo #
--error ER_GIS_INVALID_DATA
...
--echo #
--echo # Bug #29047811 ASSERTION FAILED:
--echo #               CURRENT_THD->IS_ERROR(), SQL\ITEM_GEOFUNC.CC, LINE 5358
--echo #
...
--echo # Test DEFAULT for GEOMETRY columns with no default defined.
