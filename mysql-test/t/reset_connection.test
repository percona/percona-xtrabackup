...
--echo #
--echo # Bug#30381098: COM_RESET_CONNECTION DOES NOT RESPECT CLIENT_INTERACTIVE
--echo #

SET @saved_wait_timeout = @@global.wait_timeout;
SET @saved_interactive_timeout = @@global.interactive_timeout;

SET GLOBAL interactive_timeout = @@global.wait_timeout + 1000;
connect(con_b30381098,localhost,root,,,,,INTERACTIVE);

--echo # Test: should return 1000 (interactive timeout)
SELECT @@session.wait_timeout - @@global.wait_timeout;

--echo # Reset connection
--reset_connection

--echo # Test: should still return 1000 (interactive timeout)
SELECT @@session.wait_timeout - @@global.wait_timeout;

# Cleanup
connection default;
disconnect con_b30381098;
