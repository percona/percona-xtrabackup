...
--echo # Bug#34666531: Mysqld failure - String::ptr()
...
--echo # Bug#35963172: HLL query crashes mysqld at setup_fields(assertion)
...
--echo # Bug#36892797 - Simple select crashes my Heatwave instance
DELIMITER |;
CREATE FUNCTION sys.SESS_VAR_COUNT() RETURNS INT SQL SECURITY INVOKER DETERMINISTIC BEGIN
DECLARE ses_var_count INT DEFAULT NULL;
SELECT COUNT(*) INTO @ses_var_count2 FROM performance_schema.session_variables;
IF @ses_var_count2 < 100 THEN
  SELECT 0 INTO ses_var_count;
ELSE
  SELECT @ses_var_count2 INTO ses_var_count;
END IF;
RETURN ses_var_count;
END; |
CREATE FUNCTION sys.SESS_VAR(input LONGTEXT) RETURNS LONGTEXT SQL SECURITY INVOKER DETERMINISTIC BEGIN
  DECLARE result VARCHAR(255) DEFAULT NULL;
  DECLARE ses_var_count INT DEFAULT NULL;
  SELECT sys.SESS_VAR_COUNT() INTO ses_var_count;
  SELECT CONCAT(CAST(ses_var_count AS CHAR(100)), input) INTO result;
  return result;
END; |
DELIMITER ;|
...
SET @query = "What is MySQL?";
SET @query2 = "What is HeatWave?";
SELECT metaphon(CONCAT(sys.SESS_VAR(@query), sys.SESS_VAR(@query2)));

DROP FUNCTION sys.SESS_VAR_COUNT;
DROP FUNCTION sys.SESS_VAR;
DROP FUNCTION metaphon;

--echo
--echo Bug#37839325 Assertion `!null_value || is_nullable()' failed
--echo
...
INSERT INTO t1 VALUES (1001,'BAL');

SELECT
my_median ( CAST( c1 AS UNSIGNED ) )
FROM t1
WHERE
( STRCMP( ( CAST(NULL AS BINARY) ) , c2 ) ) ;
...
SELECT
avgcost ( CAST( c1 AS UNSIGNED ) , c1 )
FROM t1
WHERE
( STRCMP( ( CAST(NULL AS BINARY) ) , c2 ) ) ;
