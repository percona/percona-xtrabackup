...
# This test doesn't support GTID as it uses MyISAM
...
--echo # Now test for I_S query which does not acquire any MDL lock.
--echo #
--echo # Query below should not be blocked.
...
--echo #
--echo # Let us also check queries to I_S wait for conflicting metadata
...
--echo # Check the I_S query which does not acquire any MDL locks is not blocked.
--echo # Sending:
...
--echo # Sending:
--send rename table t1 to t2, t3 to t1, t2 to t3;

--echo # Switching to connection 'default'.
connection default;
--echo # Waiting until RENAME TABLE is blocked.
let $wait_condition=
  select count(*) = 1 from information_schema.processlist
  where state = "Waiting for table metadata lock" and
...
SET DEBUG_SYNC= 'after_wait_locked_schema_name SIGNAL locked WAIT_FOR blocked';
--echo # Sending:
--send ALTER DATABASE db1 DEFAULT CHARACTER SET utf8mb3
...
--echo # Connection default
connection default;
--echo # Reaping: ALTER DATABASE db1 DEFAULT CHARACTER SET utf8mb3
--reap
...
--echo #
--echo # Bug#26770836 SLAVE HANGS - WAITING FOR TABLE METADATA LOCKS
--echo # Check that ALTER TABLE will not release MDL locks that were
--echo # acquired during view metadata update, until end of statement.
--echo #
...
--echo # Run ALTER TABLE until after the view metadata is updated.
--send ALTER TABLE t1 DROP COLUMN f2

connect (con2,localhost,root,,);
connection con2;
SET DEBUG_SYNC= 'now WAIT_FOR after_view_update';

--echo # Run REPAIR which tries to acquire MDL X on t2, and expect it to wait.
--send REPAIR TABLE t2;

connection default;
--echo # The wait_condition makes sure that the lock on table t2 is
--echo # held by ALTER TABLE after updating view metadata. Wait condition
--echo # fails if run without the fix.
let $wait_condition=
  select count(*) = 1 from information_schema.processlist
  where state = "Waiting for table metadata lock" and
