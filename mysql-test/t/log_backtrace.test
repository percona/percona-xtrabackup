...
--echo #
--echo # Bug#38456164: On MDS, backtraces from the traditional error are not always available in the other (e.g. JSON) logs
--echo #
--echo

# This is basically the same test, but with a different log file.

#
# Use an error-log file we prepared earlier that is larger than our buffer.
# This is a traditional error-log file, i.e. it's not JSON-formatted.
--let LOG_STACKDUMP = bug38456164_stackdump.log
--let SRC = $MYSQLTEST_VARDIR/std_data/$LOG_STACKDUMP
--let TGT = $MYSQL_TMP_DIR/$LOG_STACKDUMP
--copy_file $SRC $TGT
