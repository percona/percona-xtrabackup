...
# gtid_mode default is ON, so after restart we should again set it to OFF
--source include/rpl/set_gtid_mode_off.inc
...
SELECT id, HEX(name), LENGTH(name), CHAR_LENGTH(name) FROM t1;
SELECT id, HEX(fname), LENGTH(fname), CHAR_LENGTH(fname) FROM t2;

--echo # Should not get any rows here
SELECT t1.name, t2.fname FROM t1, t2 WHERE t1.name <> t2.fname;

--error ER_ROW_IS_REFERENCED_2
UPDATE t1 SET name=CONCAT('Wroc', UNHEX('C582') ,'aw') WHERE id = 1;

INSERT INTO t1(name) VALUES ('test2');
INSERT INTO t2(name, fname) VALUES ('test2', 'test2');
...
INSERT INTO t1(name) VALUES ('unrelated');
--echo # Note that updating a t1.name value to a string that cannot be
--echo # converted from utf8mb4 to latin1 also fails here even if the row
--echo # being updated is not related to any t2 rows.
--error ER_ROW_IS_REFERENCED_2
UPDATE t1 SET name=CONCAT('Wroc', UNHEX('C582') ,'aw') WHERE name='unrelated';
SELECT * FROM t1;
SELECT * FROM t2;
...
connection con1;
--echo # Reveals parent information because fk2 is checked before fk3
--error ER_NO_REFERENCED_ROW_2
INSERT INTO t2 (fld1, fld2) VALUES (1, 2);

--echo # Warning also reveals parent table information.
INSERT IGNORE INTO t2 (fld1, fld2) VALUES (1, 2);
SHOW WARNINGS;
...
# Verify that foreign key checking behaves correctly when the referencing table
# has a secondary index that is a partial match for the primary key of the
# referenced table.
CREATE TABLE parent (
  p1 int NOT NULL,
  p2 tinyint NOT NULL,
  ...
  p4 char (10),
  p5 char(10),
  PRIMARY KEY (p3,p2,p1),
  KEY idx_parent (p3,p2,p5,p4)
) ;
...
CREATE TABLE child (
  ...
  KEY idx_child (c3,c2,c4,c1),
  CONSTRAINT fkey_c_p FOREIGN KEY (c3, c2, c4) REFERENCES parent (p3, p2, p1)
);

--error ER_NO_REFERENCED_ROW_2
INSERT INTO child (c1,c2,c3,c4) VALUES (1,2,3,4);

INSERT INTO parent (p1,p2,p3,p4,p5) VALUES (4,2,3,'four','five');
INSERT INTO child (c1,c2,c3,c4) VALUES (1,2,3,4);

--error ER_ROW_IS_REFERENCED_2
DELETE FROM parent WHERE p1=4 AND p2=2 AND p3=3;

DELETE FROM child WHERE c1=1 AND c2=2 and c3=3;
DELETE FROM parent WHERE p1=4 AND p2=2 AND p3=3;
