...
SELECT user, host, LENGTH(authentication_string)>0 FROM mysql.user WHERE host = 'h';
# Drop the created users (cleanup).
DROP USER u1@h;
...
call mtr.add_suppression("REVOKE/GRANT failed .* An incident event has been written to the binary log which will stop the replicas.");
...
# Start will restore the gtid_mode to ON so setting it to OFF again
--source include/rpl/set_gtid_mode_off.inc
...
# Restart will restore the gtid_mode to ON so setting it to OFF again
--source include/rpl/set_gtid_mode_off.inc
