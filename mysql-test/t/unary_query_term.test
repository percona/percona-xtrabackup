...
--echo #
--echo # Bug#35058815 MySQL: Allowing unknown columns in ORDER BY clause
--echo #              attached to QT_UNARY present as QT_UNION child
--echo #
CREATE TABLE t_i1(i1 int);
CREATE TABLE t_i2(i2 int);
CREATE TABLE t_d(d DOUBLE);
INSERT INTO t_i1 VALUES (1), (3), (5), (7), (9);
INSERT INTO t_i2 VALUES (0), (2), (4), (6), (8);
INSERT INTO t_d VALUES (0.1), (2.1), (4.1), (6.1), (8.1);
ANALYZE TABLE t_i1, t_i2, t_d;

--error ER_BAD_FIELD_ERROR
((SELECT i1 FROM t_i1 ORDER BY i1 DESC LIMIT 4) ORDER BY i1 ASC LIMIT 2)
UNION DISTINCT
((SELECT i2 FROM t_i2 ORDER BY i2 DESC LIMIT 4) ORDER BY i1 ASC LIMIT 2)
ORDER BY i1;

((SELECT i1 FROM t_i1 ORDER BY i1 DESC LIMIT 4) ORDER BY i1 ASC LIMIT 2)
UNION DISTINCT
((SELECT i2 FROM t_i2 ORDER BY i2 DESC LIMIT 4) ORDER BY i2 ASC LIMIT 2)
ORDER BY i1;

--error ER_BAD_FIELD_ERROR
((SELECT i1 FROM t_i1 ORDER BY i1 DESC LIMIT 4) ORDER BY i1 ASC LIMIT 2)
UNION DISTINCT
((SELECT i2 AS foo FROM t_i2 ORDER BY i2 DESC LIMIT 4) ORDER BY i2 ASC LIMIT 2)
ORDER BY i1;

((SELECT i1 FROM t_i1 ORDER BY i1 DESC LIMIT 4) ORDER BY i1 ASC LIMIT 2)
UNION DISTINCT
((SELECT i2 AS foo FROM t_i2 ORDER BY i2 DESC LIMIT 4) ORDER BY foo ASC LIMIT 2)
ORDER BY i1;

# Verify that mixed type operation work with the above constructions

((SELECT i1 FROM t_i1 ORDER BY i1 DESC LIMIT 4) ORDER BY i1 ASC LIMIT 2)
UNION DISTINCT
((SELECT d FROM t_d ORDER BY d DESC LIMIT 4) ORDER BY d ASC LIMIT 2)
ORDER BY i1;
