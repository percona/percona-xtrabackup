...
--echo #
--echo # Bug#34800905 - View definition changes latin 1 Invalid utf8mb4 character
--echo #
...
SET NAMES DEFAULT;
--echo # Without fix, invalid character string error is reported for column c2.
SHOW CREATE VIEW v1;
--echo # Without fix, incorrect values listed for column c2.
SELECT * FROM v1;
...
--echo #
--echo # BUG#33200087: DROP VIEW IF EXISTS results in error when having a table
--echo #               with the same name
--echo #

CREATE TABLE foo1 (a BOOL);
--echo # Without the patch, the following query reports an error.
DROP VIEW IF EXISTS foo1;

CREATE TABLE foo2 (b BOOL);
CREATE VIEW v1 AS SELECT * FROM foo2;
--echo # Without the patch, the following query reports an error while trying to
--echo # drop `foo1`. Thus the view v1 is not dropped even though it exists.
DROP VIEW IF EXISTS foo1, v1;

# Confirming that the view v1 was dropped.
--let $assert_text= View v1 should not exist
--let $assert_cond= "[SELECT COUNT(*) FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_NAME=\'v1\']" = 0
--source include/assert.inc

DROP TABLE foo2, foo1;

--echo #
--echo # Bug#38138412 CREATE VIEW causes Assertion `(tlen % 2) == 0'
--echo #              in strings/ctype-ucs2.cc
--echo #

CREATE TABLE t1 (
  id INT,
  c5 VARCHAR(5) CHARACTER SET utf16
 );

INSERT INTO t1 VALUES(42, "hello");

CREATE VIEW v1 AS SELECT * FROM t1 WHERE c5 BETWEEN '' AND '4';
