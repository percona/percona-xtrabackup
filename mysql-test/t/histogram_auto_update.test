...
# Get the selectivity of the topmost filter in a query plan.
CREATE FUNCTION get_filter_percentage(plan JSON)
  RETURNS DECIMAL(5, 2) DETERMINISTIC NO SQL
  RETURN
   ROUND(
     100 *
     JSON_VALUE(plan, '$.query_plan.estimated_rows' RETURNING DOUBLE) /
     JSON_VALUE(plan, '$.query_plan.inputs[0].estimated_rows' RETURNING DOUBLE),
     2);
...
SELECT get_filter_percentage(@v) AS should_be_33;
DROP TABLE t1;
...
EXPLAIN FORMAT=JSON INTO @v SELECT x FROM t1 WHERE x > 0;
SELECT get_filter_percentage(@v) AS should_be_100;
DROP TABLE t1;
...
SELECT get_filter_percentage(@v) AS should_be_50;
DROP TABLE t1;

DROP FUNCTION get_filter_percentage;
