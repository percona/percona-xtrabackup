#
# Tests with the utf8mb3 character set
#
...
SET collation_connection='utf8mb3_bin';
-- source include/ctype_filesort.inc
-- source include/ctype_like_escape.inc
...
SET sql_mode = default;
#
# Bug#11591: CHAR column with utf8mb3 does not work properly
...
-- enable_result_log
-- enable_query_log
EXPLAIN SELECT * FROM crashtest ORDER BY CHAR(crash USING utf8mb3);
SELECT * FROM crashtest ORDER BY CHAR(crash USING utf8mb3);
...
--echo #
--echo # Bug#57687 crash when reporting duplicate group_key error and utf8mb3
...
--echo #
# Emulate utf8mb3 client erroneously started with --default-character-set=latin1,
# as in the bug report. EXPLAIN output should still be pretty readable
SET NAMES latin1;
EXPLAIN SELECT 'abcdÁÂÃÄÅ', _latin1'abcdÁÂÃÄÅ', _utf8mb3'abcdÁÂÃÄÅ' AS u;
# Test normal utf8mb3
SET NAMES utf8mb3;
EXPLAIN SELECT 'abcdÁÂÃÄÅ', _latin1'abcdÁÂÃÄÅ', _utf8mb3'abcdÁÂÃÄÅ';
...
--echo #
--echo # Bug#22531111 FUNC CONVERT AND CONVERT(,BINARY) WORKS
--echo #              INCORRECT W/ MULTI-BYTE+MULTI-ROWS
--echo #
...
--echo #
--echo # Bug#26475237: "INCORRECT STRING VALUE" IS DIPLAYED INSTEAD OF
--echo #               "DATA TOO LONG" SOMETIMES.
--echo #
...
--echo # Without the patch, returns an inappropriate error.
--error ER_DATA_TOO_LONG
