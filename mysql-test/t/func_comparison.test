...
while ($i < 19)
{
  ...
  eval $query;
--disable_query_log
  eval prepare s1 from "$prep_query1";
--enable_query_log
...
--enable_query_log
  ...
  eval $query;
--disable_query_log
  eval prepare s2 from "$prep_query2";
--enable_query_log
...
--enable_query_log
  ...
  eval $query;
--disable_query_log
  eval prepare s3 from "$prep_query3";
--enable_query_log
...
--enable_query_log
  ...
  eval $query;
--disable_query_log
  eval prepare s4 from "$prep_query4";
--enable_query_log
...
--enable_query_log
  ...
  eval $query;
--disable_query_log
  eval prepare s5 from "$prep_query5";
--enable_query_log
...
--enable_query_log
  ...
  eval $query;
--disable_query_log
  eval prepare s6 from "$prep_query6";
--enable_query_log
...
while ($i < 5)
{
  ...
  eval set @dsv= "$d_value";
  eval set @tssv= "$ts_value";
  eval set @iv= $i_value;
  eval set @dcv= $dc_value;
...
--echo # Bug#36570474: Assertion failed: !current_thd->is_error() &&
--echo #               has_value in Item_func_coalesce::val_json()

CREATE FUNCTION f(a CHAR(1)) RETURNS JSON DETERMINISTIC
  RETURN NULL;

--error ER_DATETIME_FUNCTION_OVERFLOW
SELECT f(1 ^ ISNULL(ADDDATE(CURRENT_TIMESTAMP, INTERVAL 999999999999 DAY)) AND
        FOUND_ROWS()) <=>
       ANY_VALUE(JSON_OBJECTAGG('{}','{}'));

DROP FUNCTION f;


--echo #
--echo # Bug #37462769: NULL-safe equals operator (<=>) exhibits incorrect behavior 
--echo # when comparing ROW values containing NULL
--echo #

CREATE TABLE t1ns(id INT KEY AUTO_INCREMENT, `col_smallint` SMALLINT, `col_int` INT, `col_float` FLOAT);
INSERT INTO t1ns(col_smallint, col_int, col_float) VALUES(NULL, 3, NULL);

# Should return no rows since values are not equal
SELECT `col_smallint`, `col_int`, `col_float` FROM `t1ns`
  WHERE (`col_smallint`, `col_int`, `col_float`) <=> (SELECT NULL, 1, NULL);
SELECT `col_smallint`, `col_int`, `col_float` FROM `t1ns`
  WHERE (`col_int`, `col_smallint`, `col_float`) <=> (SELECT 1, NULL, NULL);

# Should return one row
SELECT `col_smallint`, `col_int`, `col_float` FROM `t1ns`
  WHERE (`col_smallint`, `col_int`, `col_float`) <=> (SELECT NULL, 3, NULL);
SELECT `col_smallint`, `col_int`, `col_float` FROM `t1ns`
  WHERE (`col_int`, `col_smallint`, `col_float`) <=> (SELECT 3, NULL, NULL);
