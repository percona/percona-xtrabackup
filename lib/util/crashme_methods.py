...
"""
    ...
    def prepare_config(test_executor):
        """ Create the config file crash-me needs to execute """

        output_filename= "%s/crashme.cfg" % (test_executor.system_manager.workdir)
    ...
    def execute_crashme(test_cmd, test_executor, servers):
        ...
        process_crashme_output(bot)


    def process_crashme_output(bot):
        if bot.current_test_retcode == 0:
            infile_name = bot.current_test_output.split('\n')[3].split(':')[1].strip()
